...
CREATE TRIGGER `replace_id_to_subtable_runs_speak` 
...
CREATE TRIGGER validate_overt_covert_before_insert_to_runs_speak
  ...
  FOR EACH ROW
BEGIN
  IF NEW.overt_covert IS NOT NULL AND
    BINARY NEW.overt_covert NOT IN (
...
CREATE TABLE `runs_sensorimotor` (
  `run_id` int(11) DEFAULT NULL,
  `body_part` text DEFAULT NULL COMMENT 'Body Part',
  `left_right` text DEFAULT NULL COMMENT 'Laterality',
  `execution_imagery` text DEFAULT NULL COMMENT 'Execution / Imagery',
  UNIQUE KEY `run_id` (`run_id`),
...
CREATE TRIGGER `replace_id_to_subtable_runs_sensorimotor` 
  AFTER UPDATE ON `runs` 
  FOR EACH ROW
BEGIN
  IF NEW.task_name <> OLD.task_name AND
...
CREATE TRIGGER validate_body_part_before_insert_to_runs_sensorimotor
  ...
  FOR EACH ROW
BEGIN
  IF NEW.body_part IS NOT NULL AND
    BINARY NEW.body_part NOT IN (
    SELECT allowed_value FROM allowed_values
...
CREATE TRIGGER validate_left_right_before_insert_to_runs_sensorimotor
  ...
  FOR EACH ROW
BEGIN
  IF NEW.left_right IS NOT NULL AND
    BINARY NEW.left_right NOT IN (
    SELECT allowed_value FROM allowed_values
    WHERE table_name = 'runs_sensorimotor'
...
CREATE TRIGGER validate_execution_imagery_before_insert_to_runs_sensorimotor
  ...
  FOR EACH ROW
BEGIN
  IF NEW.execution_imagery IS NOT NULL AND
    BINARY NEW.execution_imagery NOT IN (
    SELECT allowed_value FROM allowed_values
    WHERE table_name = 'runs_sensorimotor'
...
CREATE TABLE `runs_mario` (
  `run_id` int(11) DEFAULT NULL,
  `velocity` text DEFAULT NULL COMMENT 'Velocity',
  UNIQUE KEY `run_id` (`run_id`),
...
CREATE TRIGGER `replace_id_to_subtable_runs_mario` 
  AFTER UPDATE ON `runs` 
  FOR EACH ROW
BEGIN
  IF NEW.task_name <> OLD.task_name AND
